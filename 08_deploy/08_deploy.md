...
Самое простое решение это вручную копировать файлы, рестартовать инстансы и нажимать кнопки в Web UI для управления топологией.
...
Поключим репозиторий Tarantool:

```bash
curl -L https://tarantool.io/jYuDdOV/release/2.8/installer.sh | bash
```

Версия `2.8` указана потому что локально установлен Tarantool 2.8, он же и прописан в dependencied у пакета.
...
* `TARANTOOL_WORKDIR` (`/var/lib/tarantool/<app-name>.<instance-name>`) - путь к wotking directory инстанса, где хранятся снапшоты и WAL логи;
...
Готовые роли можно импортить из Ansible Galaxу или подключать сабмодулями.
...
* конфигнурировать авторизацию;
* загружать конфигурацию приложения;
* конфигурировать faiilover и стартовать инстанс stateboard (внешнего хранилища конфигурации);
...
Такой подход позволяет управлять инстансами в отдельности, например рестартовать их в определенном порядке или использовать механизм `serial`, чтобы запускать плейбуки на инстансах небольшими "пачками".
...
В ридми должна содержаться инструкция такого вида:
