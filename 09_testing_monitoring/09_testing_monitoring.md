...
local server = luatest.Server:new({
    ...
    args = {'--no-bugs', '--fast'},
    -- additional env vars to pass to process
...
Основные возможности:
  ...
  * Подходит не только для юнит, но и для интеграционных тестов;
...
Откуда берутся данные? Есть две стратегии - Push и Pull - т.е. само приложение может присылать
