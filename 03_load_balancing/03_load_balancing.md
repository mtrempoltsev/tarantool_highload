...
Балансировка нагрузки бывает двух типов:
 ...
 Перечень подобного программного обеспечения в настоящее время довольно широк.
...
состоящий из входных и вычислительных узлов. Распределение нагрузки осуществляется при помощи специального алгоритма.
...
сервисов с одинаковыми адресами в территориально различных регионах Интернета. Балансировка по территориальному признаку также используется во многих CDN.
...
Алгоритм достаточно прост, но имеет целый ряд существенных недостатков недостатков.
...
Более эффективную балансировку обеспечивают другие методы, в которых при планировании и распределении нагрузки учитывается большее количество параметров.
...
Описанную проблему можно решить с помощью алгоритма, известного под названием least connections (сокращённо — leastconn). Он учитывает количество подключений, поддерживаемых серверами в текущий момент времени.
...
Запросы с этих IP направляются на «родной» сервер, если он не загружен полностью. В противном случае запрос будет перенаправлен на другой сервер (он должен быть загружен менее чем наполовину).

В алгоритме Locality-Based Least Connection Scheduling with Replication Scheduling каждый IP-адрес или группа IP-адресов закрепляется не за отдельным сервером, а за целой группой серверов.
...
Алгоритм Destination Hash Scheduling был создан для работы с кластером кэширующих прокси-серверов, но он часто используется и в других случаях.
...
Написать балансировщик на Tarantool:
  ...
  - Ограничить количество одновременных запросов, приходящих на балансировщик (по умолчанию 1000, но сделать конфигурируемым). Возвращать status code 429 при превышении лимита;
  - Предусмотреть возможность динамически (через консоль) добавлять и удалять сервера.

1. Сценарии
1. Алгоритмы балансировки нагрузки
1. Пишем свой балансировщик
1. HAProxy
1. Nginx
1. Лимит запросов в секунду (ДЗ)
